...
class _MyRequestsScreenState extends State<MyRequestsScreen> {
  ...
  Future<void> _fetchMyRequests() async {
    try {
      ...
      if (requests != null) {
        setState(() {
          _myRequestsToOthers = requests.where((r) => r['senderId'] == widget.userId).toList();
          _othersRequestsToMe = requests.where((r) => r['receiverId'] == widget.userId).toList();
          _isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> _updateRequestStatus(int requestId, int statusId) async {
    try {
      ...
      var request = _myRequestsToOthers.firstWhere(
        ...
        orElse: () => null
      ) ?? _othersRequestsToMe.firstWhere(
        (r) => r['requestId'] == requestId, 
        orElse: () => null
      );
      ...
      bool success = await RequestsService.updateRequest(requestId, updatedData);
      ...
      if (success) {
        ...
        setState(() {
          ...
          if (_myRequestsToOthers.contains(request)) {
            var index = _myRequestsToOthers.indexWhere((r) => r['requestId'] == requestId);
            if (index != -1) {
              _myRequestsToOthers[index]['statusId'] = statusId;
            }
          } else if (_othersRequestsToMe.contains(request)) {
            var index = _othersRequestsToMe.indexWhere((r) => r['requestId'] == requestId);
            if (index != -1) {
                ...
                children: [
                  ...
                  if (_myRequestsToOthers.isNotEmpty) ...[
                    ...
                    ..._myRequestsToOthers.map((request) {
                      ...
                      final formattedTime = timeString != 'N/A'
                          ? DateFormat('dd MMMM yyyy HH:mm', 'tr').format(DateTime.parse(timeString))
                          : 'N/A';
                      ...
                      return Padding(
                        ...
                        child: Card(
                          ...
                          child: Stack(
                            children: [
                              ListTile(
                                title: Text('${request['receiver']['name']} ${request['receiver']['surname']}'),
                                subtitle: Text(formattedTime),
                                contentPadding: const EdgeInsets.all(16),
                              ),
                              _buildStatusButton(request['status']['statusName']),
                            ],
                          ),
                        ),
                      );
                  ...
                  if (_othersRequestsToMe.isNotEmpty) ...[
                    ...
                    ..._othersRequestsToMe.map((request) {
                      ...
                      if(request['statusId'] == 2) {status = "Onaylandı";}
                      else if(request['statusId'] == 3) {status = "Reddedildi";}
                      ...
                      return Padding(
                        ...
                        child: Card(
                          ...
                          child: ListTile(
                            title: Text('${request['sender']['name']} ${request['sender']['surname']} ($status)'),
                            subtitle: Text(formattedTime),
                            trailing: Row(
                              ...
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () =>_updateRequestStatus(request['requestId'], 2),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.cancel, color: Colors.red),
                                  onPressed: () => _updateRequestStatus(request['requestId'], 3),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.chat_bubble, color: Colors.blue),
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            ChatScreen(postOwnerName: request?['sender']?['username'] ?? 'Unknown',
                                                      receiverId: request?['sender']?['userId'] ?? 0),
                                      ),
                                    );
                                  },
                                ),
